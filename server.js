...
app.get('/', (req, res) => {
    ...
    // Check if user is already logged in before redirecting to login
    if (req.session && req.session.loggedIn === true) {
        res.redirect('/dashboard.html'); // If logged in, go to dashboard
    } else {
        res.sendFile(path.join(__dirname, 'login.html')); // If not logged in, send login page
    }
});
// =====================================

// خدمة صفحة تسجيل الدخول وملفاتها الأساسية بدون حماية (redundant due to '/' handler, but safe)
